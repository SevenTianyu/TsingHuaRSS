        ...
        <div class="newList">
          <div class="model morning" >
            <ul style="padding: 0">
              ...
              <li v-for="story in stories" :key="story.id"  class="new border-1px" v-if="story.articleURL"
                  @click="goNew(story.articleURL)" :class="model">

                  
                  <div class="biaoti">
                    <span class="img_gzh"><img :src="attachImageUrl(story.oaImage)"
                 > </span><span class="name_gzh">{{story.oaName}} </span>
                  </div>
                  <div class="zhuti">
                     <span class="title">{{story.articleName}}</span>
                     <span class="avatar" v-if="story.articleImage" ><img :src="attachImageUrl(story.articleImage)"
                 ></span>
                  </div>
                  <div class="date">{{story.articleDate}} </div>
              </li>
            </ul>
          </div>
        </div>
    ...
    methods: {
      ...
      fetchData() {
        ...
        getNews().then((response) => {
          let stories = response.data.RECORDS;
